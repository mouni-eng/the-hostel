...
class ScanQrCodeView extends StatefulWidget {
  const ScanQrCodeView({Key? key, required this.booking}) : super(key: key);
...
class _ScanQrCodeViewState extends State<ScanQrCodeView> {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      setState(() {
        result = scanData;
        if (result!.code == widget.booking.apUid) {
          BookingService()
              .updateBookingStatus(widget.booking.user!.personalId!)
              .then((value) {
            Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => OwnerLayoutView()),
                (route) => false);
          });
        }
      });
